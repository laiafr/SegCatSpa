...
python extract_childes_info.py $INPUT_CORPUS $INPUT_FILES
...
echo "finding out who's a speaker in $f"
		...
		bash cha2sel_withinputParticipants.sh $f ${RES_FOLDER}/${SELFILE} $IncludedParts

 		ORTHO=$(basename "$name" .cha)"-ortholines.txt"
		bash selcha2clean.sh ${RES_FOLDER}/${SELFILE} ${RES_FOLDER}/$ORTHO

...
all_tags="$train_input_dir/*tags.txt"
...
do
    ...
    train="$train_input_dir/$(basename $tags)"
    echo -n "[$counter/$ntags] building jobs for $name testing and $train train..."

    # defines segmentation jobs
#    echo "$name-syllable-tprel $tags syllable $separator wordseg-tp -v -t relative --train-file $train " >> $jobs
#    echo "$name-syllable-tpabs $tags syllable $separator wordseg-tp -v -t absolute --train-file $train " >> $jobs
    echo "$name-phone-dibs $tags phone $separator wordseg-dibs -v -t phrasal -u phone --train-file $train " >> $jobs
#    echo "$name-phone-puddle $tags phone $separator wordseg-puddle -v  -w 2 --train-file $train " >> $jobs
#    echo "$name-syllable-ag $tags syllable $separator wordseg-ag -vv -j 8 --train-file $train " >> $jobs
